...
impl Driver {
    ...
    /// Returns the current DAVE Privacy Code for the connection, if any.
    ///
    /// This code can be used to verify the end-to-end encryption with other
    /// participants in the voice channel.
    #[instrument(skip(self))]
    pub async fn get_privacy_code(&self) -> Option<String> {
        let (tx, rx) = tokio::sync::oneshot::channel();
        if self.sender.send(CoreMessage::GetPrivacyCode(tx)).is_ok() {
