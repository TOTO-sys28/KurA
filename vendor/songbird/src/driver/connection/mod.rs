...
use crate::model::payload::Identify;
    ...
    pub(crate) async fn new_inner(
        ...
        #[cfg(feature = "dave")]
        {
            let identify = serde_json::json!({
                "op": 0,
                "d": {
                    "server_id": info.guild_id.0.get().to_string(),
                    "user_id": info.user_id.0.get().to_string(),
            ...
            client
                .send_json_text(
                    &serde_json::to_string(&identify).map_err(Error::Json)?,
                )
                .await?;
        }
        #[cfg(not(feature = "dave"))]
        ...
        #[cfg(feature = "dave")]
        let (maybe_secret, dave_session) = crate::driver::dave::perform_dave_handshake(
            &mut client,
            &info,
            &ws_msg_tx,
        )
        ...
        #[cfg(not(feature = "dave"))]
        let maybe_secret = None;

        let cipher = init_cipher(&mut client, chosen_crypto, &ws_msg_tx, maybe_secret).await?;
        ...
        let mix_conn = MixerConnection {
            ...
            udp_tx,
            #[cfg(feature = "dave")]
            dave_session: Some(dave_session.clone()),
        };
...
async fn init_cipher(
    ...
    mode: CryptoMode,
    tx: &Sender<WsMessage>,
    maybe_secret: Option<Vec<u8>>,
) -> Result<Cipher> {
    // If an external DAVE handshake provided the secret key, use it directly.
    if let Some(secret) = maybe_secret {
        return mode
            .cipher_from_key(&secret)
