...
impl Mixer {
    ...
    fn prep_packet(&mut self, mix_len: MixType, packet: &mut [u8]) -> Result<usize> {
        ...
        #[cfg(feature = "dave")]
        if let Some(session_mutex) = &conn.dave_session {
            if let Ok(mut session) = session_mutex.lock() {
                let opus_data = payload[first_payload_byte..first_payload_byte + payload_len].to_vec();
                match session.encrypt(
                    davey::MediaType::AUDIO,
                    davey::Codec::OPUS,
                    &opus_data,
                ) {
                    Ok(encrypted_chunk) => {
                        let new_payload_len = encrypted_chunk.len();
                        payload[first_payload_byte..first_payload_byte + new_payload_len]
                            .copy_from_slice(&encrypted_chunk);
                        payload_len = new_payload_len;
                    }
                    Err(e) => {
                         tracing::warn!("DAVE media encrypt failed: {:?}", e);
                    }
                }
            }
        }
